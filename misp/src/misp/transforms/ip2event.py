...
@configure(
    label='To Event [MISP]',
    description='Returns a MISPEvent entities containing the corresponding IP address attribute"',
    uuids=[ 'misp.v2.ip2event' ],
    inputs=[ ( 'MISP', IPv4Address ) ],
    debug=True
)
def dotransform(request, response):
    ...
    complete_url = "%s/events/restSearch/%s/%s/ip-src&&ip-dst" % (url, apikey, request.value)
