...
@configure(
    label='To Event [MISP]',
    description='Returns a MISPEvent entities containing the corresponding domain/hostname attribute"',
    uuids=[ 'misp.v2.domain2event' ],
    inputs=[ ( 'MISP', Domain ) ],
    debug=True
)
def dotransform(request, response):
    ...
    complete_url = "%s/events/restSearch/%s/%s/domain&&hostname" % (url, apikey, request.value)
