...
# warning - this needs Malformity to get installed
...
@configure(
    label='To Event [MISP]',
    description='Returns a MISPEvent entities containing the corresponding hash attribute"',
    uuids=[ 'misp.v2.hash2event' ],
    inputs=[ ( 'MISP', Hash ) ],
    debug=True
)
def dotransform(request, response):
    ...
    complete_url = "%s/events/restSearch/%s/%s/md5&&sha1&&sha256" % (url, apikey, request.value)
