...
from Malformity.transforms.common.entities import Hash
...
@configure(
    label='To hash [MISP]',
    description='Returns all hash attributes belonging to an event"',
    uuids=[ 'misp.v2.event2hash' ],
    inputs=[ ( 'MISP', MISPEvent ) ],
    debug=True
)
def dotransform(request, response):
    ...
    complete_url = "%s/attributes/returnAttributes/%s/%s/md5&&sha1&&sha256" % (url, apikey, request.value)
    ...
    progress(50)
    for a in rootElement.findall('Attribute/value'):
